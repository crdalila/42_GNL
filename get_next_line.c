...
char	*read_document(char *buffer, char *saved, int fd)
{
	...
	if (ft_strlen(buffer) > 0) //si hay algo en el buffer (si hay algo de una llamada anterior)
		saved = ft_strjoin(saved, buffer);//a saved se le añade lo nuevo que hay en buffer
	while (!ft_strchr(buffer, '\n') && chars_read != 0) //mientras no haya salto de línea o hasta que ya no haya nada más que leer (chars_Read=0)
	{
		...
		saved = ft_strjoin(saved, buffer); //juntamos el saved y buffer 
		if (!saved)
		{
			free(saved);
			return (NULL);
...
char	*prepare_new_line(char *saved)
{
	...
	ft_memcpy (new_line, saved, new_size); //copia la línea original en la nueva
